    ...
    SubShader
    {
        ...
        float distScene(float3 pos, out int objectId, out float colorVariation)
        {
            ...
            if (distPot < dist)
            {
                ...
                colorVariation = 0.9 * smoothstep(0.1, 0.2, 0.5 * sin(5.0 * sin(10.0 * anglev)) + 0.3 * (f - 0.5)) + 0.1
                    * sf;
            }
            ...
            if (distPlant < dist)
            {
                ...
                colorVariation = 0.5 + 0.5 * (smoothstep(0.5, 0.9, colorVariation) - smoothstep(
                    0.55, 0.95, -colorVariation));
            ...
            radius = 0.09;
            distFlower = 1.3 * min(
                distFlower, length(pLayer - float3(0.4 * radius, -0.68 * radius, -0.67 * radius)) - radius);
        ...
        float3 render(float2 uv)
        {
            ...
            if (hit)
            {
                ...
                if (o == 3)
                {
                    col = lerp(float3(0.3, 0.7, 0.6), float3(0.85, 0.95, 0.7), c);
                    shine = 5.0;
                }
                if (o == 4)
                {
                    col = lerp(float3(0.85, 0.95, 0.7), float3(0.96, 0.6, 0.85), c);
                    shine = 5.0;
                }
