Shader "Shadertoy/wsfXDS_SousLOcean"
{
    ...
    SubShader
    {
        ...
        float3 Voro(float2 uv)
        {
            ...
            float2 m_diff = 0.0.xx;
            float2 m_point = 0.0.xx;
            float2 m_neighbor = 0.0.xx;
            float m_dist = 10.0;
        ...
        {
            float m1 = saturate(SunMask(uv, 0.01) - (GroundMask1(uv, 0.0) + GroundMask2(uv, 0.0) + SeaweedMask(uv, 0.0)));
            return RedGrid(mul(Rot(_STTime * 0.15), (uv - float2(0.4, 0.2))), 18.0) * m1;
        }

        float3 Sky(float2 uv)
        {
            float m1 = saturate(1.0 - (GroundMask1(uv, 0.0) + GroundMask2(uv, 0.0) + SeaweedMask(uv, 0.0) + SunMask(uv, 0.0)));
