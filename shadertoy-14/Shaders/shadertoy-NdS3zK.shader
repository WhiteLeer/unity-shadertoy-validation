    ...
    SubShader
    {
        ...
        float2 GetGradient(float2 uv)
        {
            ...
            return float2(data - SAMPLE_TEXTURE2D(_Channel1, sampler_Channel1, uv - float2(delta, 0)).r,
                          data - SAMPLE_TEXTURE2D(_Channel1, sampler_Channel1, uv - float2(0, delta)).
                          r);
        }
        ...
        float3 GetTriplanar(float3 position, float3 normal)
        {
            ...
            if (abs(position.z) > 0.65)
                xpos = lerp(xpos, float2(position.z, -position.x),
                                                                            smoothstep(0.0, -0.2, position.y));
            float3 axes = float3(GetDistortedTexture(position.zy), GetDistortedTexture(xpos),
                                                                      GetDistortedTexture(position.xy));
        ...
        float3 ShadingPBR(float3 cameraPos, float3 lightPos, float3 p, float3 n, float3 rd, float3 geoNormal)
        {
            ...
            result += (1.0 - f) * CLARITY * GetEnvironment(p + rd * 2.0 * EPSILON, refract(rd, n, ETA), transmittance,
                             halfway);
        ...
        {
            float3 rayDir = RayDirection(60.0, fragCoord);
            float3 cameraPos = SAMPLE_TEXTURE2D(_Channel0, sampler_Channel0, (float2(0, 1) + 0.5) / _STResolution.xy).
xyz;
