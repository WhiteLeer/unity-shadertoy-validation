Shader "Shadertoy/WdXXR8_VanGoghRaytracer"
{
    ...
    SubShader
    {
        ...
        MaterialT MaterialMake(int type, int flags, int attrib0, int attrib1, float4 ambient, float4 diffuse,
                               float4 specular, float4 emission, float4 reflection, float4 transparent)
        {
            MaterialT m;
            m.type = type;
            m.flags = flags;
        ...
        {
            if (idx == 0)
                return MaterialMake(STANDARD_MATERIAL, 0, 0, 0, float4(.1, .1, .1, 1), float4(.1, .1, .1, 1),
                                                                        float4(1, 1, 1, 100), float4(0, 0, 0, 0),
                                                                        float4(0, 0, 0, 0),
                                                                        float4(0, 0, 0, 0));
            if (idx == 1)
                return MaterialMake(STANDARD_MATERIAL, 0, 0, 0, float4(1, 1, 1, 1), float4(1, 1, 1, 1),
            ...
            if (idx == 2)
                return MaterialMake(CHECKER_MATERIAL, 0, 0, 1, float4(0.5, 0.5, 0.5, 0),
                    float4(0.001, 0.001, 0.001, 0), float4(0, 0, 0, 0), float4(0, 0, 0, 0), float4(0, 0, 0, 0),
                    float4(0, 0, 0, 0));
            if (idx == 3)
                return MaterialMake(STANDARD_MATERIAL, 0, 0, 0, float4(0, 0, 0, 1), float4(0, 0, 0, 1),
                             float4(1, 1, 1, 50), float4(0, 0, 0, 0), float4(0.2, 0.2, 0.2, 0),
                             float4(0.6, 0.6, 0.6, 1.24));
            if (idx == 4)
                return MaterialMake(STANDARD_MATERIAL, 0, 0, 0, float4(0.3, 0.3, 0.3, 1),
                                               float4(0.3, 0.3, 0.3, 1), float4(1, 1, 1, 50), float4(0, 0, 0, 0),
                                               float4(0.6, 0.6, 0.6, 0),
                                               float4(0, 0, 0, 0));
            if (idx == 5)
                return MaterialMake(STANDARD_MATERIAL, 0, 0, 0, float4(1, 0, 0, 1), float4(1, 0, 0, 1),
                    float4(1, 1, 1, 60), float4(0, 0, 0, 0), float4(0, 0, 0, 0), float4(0, 0, 0, 0));
            return MaterialMake(STANDARD_MATERIAL, 0, 0, 0, float4(.8, .6, 0, 1), float4(.8, .6, 0, 1),
                float4(0.5, 0.5, 0.5, 30), float4(0, 0, 0, 0), float4(0.1, 0.1, 0.1, 0),
                float4(0, 0, 0, 0));
        }
        ...
        PrimitiveT GetPrimitive(int i)
        {
            ...
            if (i == 2)
                return PrimitiveMake(SPHERE_PRIMITIVE, 4, float3(4, 2, -1.5), float3(1.5, 0, 0),
                float3(0, 0, 0));
        ...
        float4 Frag(Varyings i) : SV_Target
        {
            ...
            for (int a = 0; a < 2; a++)
            {
                ...
                for (int b = 0; b < 2; b++)
                {
                    float2 noise = SAMPLE_TEXTURE2D(_Channel0, sampler_Channel0, fragCoord / _STResolution.xy).xy * 2.0
                        - 1.0;
