...
public abstract class ShadertoyBootstrapBase : MonoBehaviour
{
    ...
    protected virtual string TargetShaderAssetPath => null;
...
#if UNITY_EDITOR
        if (shader == null && !string.IsNullOrWhiteSpace(TargetShaderAssetPath))
        {
            shader = AssetDatabase.LoadAssetAtPath<Shader>(TargetShaderAssetPath);
        }
#endif
        if (shader == null)
        {
