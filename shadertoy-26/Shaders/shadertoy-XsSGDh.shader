    ...
    SubShader
    {
        ...
        float4 Trace(float3 rs, float3 rd, float2 fragCoord)
        {
            ...
            for (int i = 0; i < 64; ++i)
            {
                ...
                if (h <= 0.0)
                {
                    ...
                    if (nrd < 0.0)
                    {
                        ...
                        col.xyz += col.w * fr * 3.0 * rocc * pow(
                            SampleLatLong(TEXTURE2D_ARGS(_Channel3, sampler_Channel3), rr), float3(2.2, 2.2, 2.2));
                        if (q > 0.1)
                        {
                            rd = inside *
                                normalize(refract(rd * inside, n, (inside < 0.0) ? refrIdx : (1.0 / refrIdx)));
                        }
                        inside = -inside;
                        if (inside < 0.0)
                        {
                ...
                if (inside < 0.0)
                {
                    ...
                    gcol = lerp(gcol, float3(0.5, 0.5, 0.5), -1.9);
            ...
            if (dot(rd, flo - rs) > 0.0)
            {
                ...
                bg = lerp(bg, float3(1.0, 1.0, 1.0), smoothstep(0.0, 0.1, q));
            }
        ...
        float4 RenderMarble(float2 fragCoord)
        {
            ...
            float2 m = _STMouse.xy / max(_STResolution.xy, float2(1.0, 1.0));
            ...
            col = pow(max(col, 0.0), float4(0.4, 0.4, 0.4, 0.4));
