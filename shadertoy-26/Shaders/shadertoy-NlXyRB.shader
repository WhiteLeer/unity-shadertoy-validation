Shader "Shadertoy/NlXyRB_OceanCrystal"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        float Map(float3 p, inout CrystalState st)
        {
            ...
            st.sd = length(p) - 1.3;
            st.sd = lerp(st.sd, -length(pp), 0.01);

            if (st.sd > 0.01)
            {
                st.gl += exp(-st.sd) * float3(0.0, 0.005, 0.005);
            }

            st.sd = abs(st.sd) - 0.001;
            float wave = sin(pp.x + st.tt) * 0.5;
            float m = smoothstep(-2.0 + wave, 1.0 + wave, pp.y);
            float3 col = lerp(float3(0.3, 0.3, 1.0), float3(0.2, 0.65, 1.0), m);

            if (st.sd < 0.001)
            {
                st.oc = col;
                st.io = max(cos(st.tt * 0.1 + p.x + p.y + p.z) * 0.5 + 0.5, 0.001);
                st.oa = -0.1 + pow(max(-(pp.y - 8.0) * 0.05, 0.0), 1.5);
                st.ss = float3(0.0, 0.0, 0.0);
                st.vb = float3(1.0, 8.0, 2.5);
                st.ec = 2;
            }
            return st.sd;
        }
        ...
        {
            st.cc = float3(0.5, 0.5, 0.8) + length(pow(abs(st.rd + float3(0.0, 0.5, 0.0)), 3.0)) * 0.3 + st.gl;
