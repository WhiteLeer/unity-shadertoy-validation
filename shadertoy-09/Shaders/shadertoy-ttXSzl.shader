Shader "Shadertoy/ttXSzl_Image"
{
    ...
    SubShader
    {
        ...
        float4 Frag(Varyings i) : SV_Target
        {
            ...
            {
                col = noLine
                          ? float3((diffuse < 0.2 ? 0.5 : 0.7), (diffuse < 0.2 ? 0.5 : 0.7),
                                   (diffuse < 0.2 ? 0.5 : 0.7)) + 0.3 * baseColor
                          : float3(0, 0, 0);
            }
            else if (m == 4)
            {
            ...
            else
            {
                ...
                float raster = 0.2 + 0.6 * length(
                    ModGLSL(fragCoord.xy, float2(rasterSize, rasterSize)) / rasterSize - 0.5);
